...
class featureRegression:
	...
	def getCV(self, folds = 10):
		scores = cross_val_score(self.randomForestRegr, self.trainX, self.trainY, scoring="r2", cv = folds)
		self.rfCV = scores
		print("RF Accuracy: %0.2f (+/- %0.2f)" % (scores.mean(), scores.std() * 2))
		scores = cross_val_score(self.adaBoostedRegr, self.trainX, self.trainY, scoring="r2", cv = folds)
		self.adaCV = scores
		print("Ada Accuracy: %0.2f (+/- %0.2f)" % (scores.mean(), scores.std() * 2))
		scores = cross_val_score(self.multilayerPerceptronRegr, self.trainX, self.trainY, scoring="r2", cv = folds)
		self.mlpCV = scores
