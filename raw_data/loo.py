...
for row in antigenIndex:
	...
	#Export to a file
	unknownDf.to_csv("raw_data/loo/" + str(i) + ".csv" )
	holdFrame.to_csv("raw_data/loo/" + str(i) + "_real.csv")
	
	#Increment counter
	i += 1	

#Iterate through and calculate averages and variances
outFile = open("raw_data/loo/summary.tsv","w+")
outFile.write("seq	rf	ada	mlp	ens	rfstd	adastd	mlpstd	ensstd\n")
summaryFile = open("raw_data/loo/dist_summary.tsv","w+")
summaryFile.write("seq	rf	ada	mlp	ens\n")
summaryFile.close()
i = 0
for row in antigenIndex:
	print(row)
	unknownDf = pd.read_csv("raw_data/loo/" + str(i) + ".csv", header = 0, index_col=[0])
	holdFrame = pd.read_csv("raw_data/loo/" + str(i) + "_real.csv", header = 0, index_col=[0])
	...
	summaryDf.to_csv('raw_data/loo/dist_summary.tsv', mode='a', header=False)
	i += 1
outFile.close()
...
for row in antigenIndex:
	print(row)
	predFrame = pd.read_csv("raw_data/loo/" + str(i) + ".csv", header = 0, index_col = 0)
	...
	predFrame.to_csv("raw_data/loo/" + str(i) + ".csv" )
	i += 1

#I AM HERE	
#Calculate differences in terms of distances on the map overlayed
outFile = open("raw_data/loo/dist_summary.tsv","w+")
...
for row in antigenIndex:
	...
	visualizer.iterativeMDSwithProcrustes("raw_data/loo/" + str(i) + ".csv", "Random Forest")
	...
	visualizer.iterativeMDSwithProcrustes("raw_data/loo/" + str(i) + ".csv", "AdaBoosted")
	...
	visualizer.iterativeMDSwithProcrustes("raw_data/loo/" + str(i) + ".csv", "MLP Network")
	...
	visualizer.iterativeMDSwithProcrustes("raw_data/loo/" + str(i) + ".csv", "Ensemble")
